...
def main() -> None:
    print(
        *[i[0 : i.find("#")] for i in stdin.readlines() if not i[0] == "#"],
        sep="\n",
    )
...
def main() -> None:
    ...
    print(
        *[line for line in x[:-1] if x[-1].lower() in line.lower()], sep="\n"
    )
...
def main() -> None:
    ...
    data = [
        {
            test["pattern"]: block["points"] // len(block["tests"])
...
def main() -> None:
    ...
    res = [
        (
            alphabet[(alphabet.find(x.lower()) + shift) % 26]
            if x in alphabet
            else x
        )
        for x in words
