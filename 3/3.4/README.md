...
```python
def main() -> None:
    deck, suit = [(x, y) for x in [*[i for i in range(2, 11)], "валет", "дама", "король", "туз"] for y in
                  ["пик", "треф", "бубен", "червей"]], input()
...
def main() -> None:
    for products in permutations(sorted([x for _ in range(int(input())) for x in input().split(", ")]), 3):
        print(*[" ".join(products)])
...
def main() -> None:
    ...
    card_layout_tuple = (", ".join(" ".join(card_tuple) for card_tuple in sorted(card_layout)) for card_layout in
                         card_layout_tuple)
    card_layout_list = [card_layout for card_layout in sorted(set(card_layout_tuple)) if best_suit in card_layout]
